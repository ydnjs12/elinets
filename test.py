...
test_path = f'{params.dataroot}/test'
# test_path = f'{args.source}/test'

output = args.output
output = output[:-1] if output.endswith("/") else output

weight = args.load_weights
img_path = glob(f'{test_path}/*.jpg') + glob(f'{test_path}/*.png')
...
with torch.no_grad():
    ...
    if seg_mode == BINARY_MODE:
        seg_mask = torch.where(segmentation >= 0, 1, 0)
    ...
    out = postprocess(x, classification, threshold, total_lane)
    ...
    for i in range(len(ori_imgs)):
        ...
        for j in range(len(out[i]['rois'])):
            ...
            plot_one_box(ori_imgs[i], [x1, y1, x2, y2], label=obj, score=score,
                         color=color_list[get_index_label(obj, label_list)])
